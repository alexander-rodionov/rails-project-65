...
# docker build -t rails_project_65 .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name rails_project_65 rails_project_65
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl libjemalloc2 libvips libpq-dev sqlite3 && \
...
RUN SECRET_KEY_BASE_DUMMY=1 ./bin/rails solid_queue:install db:migrate
